  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-grow flex flex-col justify-center">
      ...
      <div className="print:hidden mb-8 text-center max-w-xl mx-auto">
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Portal de Validação Pública</h1>
        <p className="text-sm text-slate-600 mt-1.5">
          Verifique credenciais educacionais com segurança. Digite o ID do certificado de 9 dígitos abaixo para pesquisar o registro de auditoria em blockchain.
        </p>
        ...
        <form onSubmit={onSubmit} className="flex gap-2 mt-6">
          <div className="relative flex-grow">
            ...
            <input
              ...
              placeholder="Digite o ID do Certificado (ex: 10293848)"
      ...
      {loading && (
        <div className="text-center py-20">
          ...
          <p className="text-sm text-slate-500 mt-3">Lendo o histórico de auditoria na blockchain...</p>
        </div>
      )}
            ...
            <div className="flex items-center gap-3">
              {result.isValid ? (
                <>
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-900 leading-tight">Verificação Bem-Sucedida</h2>
                    <span className="text-xs font-semibold text-emerald-700 uppercase tracking-wide">Credencial Válida</span>
                  </div>
                </>
              ) : result.revoked ? (
                <>
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-900 leading-tight">Aviso de Revogação</h2>
                    <span className="text-xs font-semibold text-red-700 uppercase tracking-wide">Revogada / Suspensa</span>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-900 leading-tight">Registro Não Encontrado</h2>
                    <span className="text-xs font-semibold text-amber-700 uppercase tracking-wide">Nenhum Registro Encontrado</span>
                  </div>
                </>
              )}
            </div>
          ...
          {result.exists ? (
            <div className="p-6 md:p-8 space-y-6">
              ...
              <div className="flex flex-col items-center text-center pb-6 border-b border-slate-100">
                ...
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Registro Educativo SkillChain</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Nome do Aluno</span>
                  <span className="font-semibold text-slate-800 mt-0.5 block">{result.studentName}</span>
                </div>
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Identificador do Aluno</span>
                  <span className="font-semibold text-slate-800 mt-0.5 block">{result.studentHash.substring(0, 16)}... (SHA-256)</span>
                </div>
                <div className="md:col-span-2">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Autoridade Emissora</span>
                  <span className="font-semibold text-slate-800 mt-0.5 block">
                    {result.issuerName || 'Instituição Registrada'}
                  </span>
                  <span className="font-mono text-xs text-slate-500 block">{result.issuer}</span>
                </div>
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Data de Emissão</span>
                  <span className="font-semibold text-slate-800 mt-0.5 block">
                    {new Date(result.issuedAt).toLocaleDateString('pt-BR', { dateStyle: 'long' })}
                  </span>
                </div>
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">ID da Credencial</span>
              ...
              <div className="pt-6 border-t border-slate-100 space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-4 space-y-3 font-mono text-xs">
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <span className="text-slate-500 flex-shrink-0">HASH DA TX (BLOCKCHAIN):</span>
                    <a
            ...
            <div className="p-12 text-center text-slate-500 space-y-3">
              ...
              <p className="text-sm">Não conseguimos localizar nenhum registro correspondente ao ID do Certificado <strong>{id}</strong>.</p>
              <p className="text-xs text-slate-400">Por favor, verifique o ID ou entre em contato com a autoridade emissora para verificar se o registro foi concluído.</p>
            </div>
          )}
        </motion.div>
      )}
